...
> [!IMPORTANT]
> 当前分支已支持 **Cloudflare 泛域名多主域名模式**：在后台统一维护 `*.root-domain` 域名池，创建地址时返回具体子域地址；该类地址默认 **仅收信**，并可在 **90 天** 内继续接收新邮件。
...
- [x] 完整的 admin 控制台
- [x] Cloudflare 泛域名一体化配置，支持多主域名泛解析池和启用域名快速切换
- [x] `admin` 后台创建无前缀邮箱
...
- Cloudflare 泛解析模式下，前端/后台选择的是 `*.example.com` 这类根规则，系统会自动生成具体子域地址，例如 `tmp@silverharbor.example.com`。
- 泛解析创建的地址为 **仅收信地址**；如需发信，请使用项目中单独配置的发信方案，而不是动态生成的 wildcard 地址。
