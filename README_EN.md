...
> [!IMPORTANT]
> This branch supports **Cloudflare multi-domain wildcard mode**: admins manage a shared pool of `*.root-domain` rules, address creation returns a concrete subdomain address, and wildcard-created addresses are **receive-only** for **90 days**.
...
- [x] Integrated Cloudflare wildcard settings with multi-root pools and quick active-domain switching
...
- In Cloudflare wildcard mode, the UI selects rules such as `*.example.com`, and the worker returns a concrete subdomain address such as `tmp@silverharbor.example.com`.
- Wildcard-created addresses are **receive-only**. If you need sending, use a separately configured sending address instead of a dynamically generated wildcard address.
